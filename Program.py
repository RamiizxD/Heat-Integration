...
GA_CONFIG = {
    ...
    # Area & LMTD limits to prevent huge HEX and TAC blow-up
    "A_min": 1.0,       # Minimum HEX area (m²) - avoids numerical issues
    "A_max": 500.0,     # Maximum HEX area (m²) - reject/penalize above this
    "LMTD_min": 2.0,    # Minimum LMTD (K) - reject if smaller (prevents huge area)
    "area_penalty_factor": 1e4,  # Penalty per m² over A_max (added to TAC)
}
...
def render_optimization_inputs():
    ...
    with st.expander("Area & LMTD limits (prevent huge HEX)", expanded=True):
        st.info("Limits to stop the GA from producing heat exchangers with huge areas that make TAC skyrocket.")
        area_col1, area_col2 = st.columns(2)
        with area_col1:
            a_min = st.number_input("Min HEX area (m²)", value=1.0, min_value=0.1, step=0.5, key="ga_a_min")
            a_max = st.number_input("Max HEX area (m²)", value=500.0, min_value=50, max_value=5000, step=50, key="ga_a_max")
        with area_col2:
            lmtd_min = st.number_input("Min LMTD (K) – reject if smaller", value=2.0, min_value=0.5, step=0.5, key="ga_lmtd_min")
            area_penalty = st.number_input("Penalty factor for area > max", value=1e4, min_value=1e2, step=1e3, format="%.0e", key="ga_area_penalty")
        GA_CONFIG["A_min"] = a_min
        GA_CONFIG["A_max"] = a_max
        GA_CONFIG["LMTD_min"] = lmtd_min
        GA_CONFIG["area_penalty_factor"] = area_penalty
        econ_params["A_min"] = a_min
        econ_params["A_max"] = a_max
        econ_params["LMTD_min"] = lmtd_min
        econ_params["area_penalty_factor"] = area_penalty
...
def calculate_tac_for_matches(matches, hot_streams, cold_streams, econ_params, dt_min):
    ...
    for m in matches:
        ...
        if lmtd <= 0:
            return float('inf')
        # Reject tiny LMTD (would imply huge area)
        LMTD_min = econ_params.get("LMTD_min", GA_CONFIG.get("LMTD_min", 2.0))
        if lmtd < LMTD_min:
            return float('inf')
            
        raw_area = q / (u * lmtd)
        A_min = econ_params.get("A_min", GA_CONFIG.get("A_min", 1.0))
        A_max = econ_params.get("A_max", GA_CONFIG.get("A_max", 500.0))
        area_penalty_factor = econ_params.get("area_penalty_factor", GA_CONFIG.get("area_penalty_factor", 1e4))
        # Clip area for capital; penalize if over limit so GA avoids huge HEX
        area = np.clip(raw_area, A_min, A_max)
        total_inv += calculate_hex_capital(area, econ_params)
        if raw_area > A_max:
            total_inv += area_penalty_factor * (raw_area - A_max) ** 2
    ...
    if actual_qh > 0.1:
        ...
        if u_hu > 0:
            raw_area_hu = actual_qh / (u_hu * lmtd_util)
            area_hu = np.clip(raw_area_hu, A_min, A_max)
            total_inv += calculate_hex_capital(area_hu, econ_params)
            if raw_area_hu > A_max:
                total_inv += area_penalty_factor * (raw_area_hu - A_max) ** 2
    ...
    if actual_qc > 0.1:
        ...
        if u_cu > 0:
            raw_area_cu = actual_qc / (u_cu * lmtd_util)
            area_cu = np.clip(raw_area_cu, A_min, A_max)
            total_inv += calculate_hex_capital(area_cu, econ_params)
            if raw_area_cu > A_max:
                total_inv += area_penalty_factor * (raw_area_cu - A_max) ** 2
